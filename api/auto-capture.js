...
async function fetchTranscript(videoId) {
  ...
  return { ok: false, videoId, error: 'All innertube strategies failed' };
}

// ---------------------------------------------------------------------------
// Apify fallback — uses a headless browser to bypass YouTube bot detection
// Requires APIFY_API_TOKEN env var in Vercel
// ---------------------------------------------------------------------------

async function fetchTranscriptApify(videoId) {
  const apiToken = process.env.APIFY_API_TOKEN;
  if (!apiToken) return { ok: false, videoId, error: 'APIFY_API_TOKEN not configured' };

  const actorId = 'jTeLbWgPzyrAlUqWC'; // george.the.developer/youtube-transcript-scraper
  ...
  try {
    ...
    if (!runRes.ok) {
      ...
      return { ok: false, videoId, error: `Apify HTTP ${runRes.status}: ${errText.substring(0, 200)}` };
    }

    const items = await runRes.json();
    const item = items?.[0];

    if (!item || !item.hasTranscript || !item.transcriptText) {
      return {
        ok: false,
        videoId,
        error: item?.error || 'Apify returned no transcript',
      };
    }

    // Build segments from Apify timestamped data if available, otherwise use plain text
    const transcript = item.transcriptText;
    ...
    return {
      ...
      segmentCount: 0, // Apify doesn't provide segment-level data in this actor
      transcript,
      timestamped: '', // Not available from this actor
      clientUsed: 'APIFY',
    };
  } catch (err) {
    return { ok: false, videoId, error: `Apify error: ${err.message}` };
  }
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const video of newVideos) {
      ...
      // Step 2: Fetch transcript (innertube first, Apify fallback)
      let transcriptResult = await fetchTranscript(video.videoId);

      if (!transcriptResult.ok) {
        console.log(`[auto-capture] Innertube failed for ${video.videoId}, trying Apify fallback...`);
        transcriptResult = await fetchTranscriptApify(video.videoId);
      }
      ...
      if (!transcriptResult.ok) {
        result.errors.push({
          ...
          note: 'Both innertube and Apify strategies failed',
        });
        continue;
      }
