...
function initSessions() {
  ...
  // === NEW FEATURES ===
  var quickActions=document.getElementById('session-quick-actions'),summarizeBtn=document.getElementById('session-summarize-btn'),exportBtn=document.getElementById('session-export-btn'),charCountEl=document.getElementById('session-char-count'),inlineSummary=document.getElementById('session-inline-summary');
  transcriptText.addEventListener('input',function(){var l=transcriptText.value.trim().length;if(quickActions)quickActions.style.display=l>0?'flex':'none';if(charCountEl)charCountEl.textContent=l.toLocaleString()+' characters';});
  if(summarizeBtn){summarizeBtn.addEventListener('click',async function(){var text=transcriptText.value.trim();if(text.length<50){showTranscriptMessage('Too short to summarize.','#c62828');return;}summarizeBtn.disabled=true;summarizeBtn.textContent='Analyzing...';inlineSummary.style.display='';inlineSummary.innerHTML='<div style="text-align:center;padding:24px"><div style="width:36px;height:36px;border:3px solid #E0DCD8;border-top-color:#E98300;border-radius:50%;margin:0 auto 12px;animation:aiSpin 1s linear infinite"></div><p style="color:#6B7B8D">Extracting key points...</p></div>';try{var r=await fetch('/api/summarize',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({transcript:text,title:saveLabel.value.trim()||'Session'}),signal:AbortSignal.timeout(65000)});var d=await r.json();if(!d.ok)throw new Error(d.error);var h='<div style="display:flex;justify-content:space-between;margin-bottom:.75rem"><h4 style="margin:0;font-size:1rem;color:#2E4057">AI Summary</h4><button id="inline-summary-close" style="background:none;border:none;font-size:1.2rem;cursor:pointer;color:#9EA8B4">&times;</button></div>';var a=d.analysis;if(a.summary)h+='<p style="font-size:.88rem;color:#2E4057;line-height:1.65">'+escHtml(a.summary)+'</p>';if(a.keyPoints&&a.keyPoints.length){h+='<h5 style="font-size:.92rem;font-weight:700;color:#8C1515;margin:.75rem 0 .5rem">Key Points</h5><ol style="margin:0;padding-left:1.25rem">';a.keyPoints.forEach(function(p){h+='<li style="font-size:.88rem;color:#2E4057;margin-bottom:.4rem">'+escHtml(p)+'</li>';});h+='</ol>';}if(a.clinicalPearls&&a.clinicalPearls.length){h+='<h5 style="font-size:.92rem;font-weight:700;color:#8C1515;margin:.75rem 0 .5rem">Clinical Pearls</h5><ul style="margin:0;padding-left:1.25rem">';a.clinicalPearls.forEach(function(p){h+='<li style="font-size:.88rem;color:#2E4057;margin-bottom:.4rem">'+escHtml(p)+'</li>';});h+='</ul>';}inlineSummary.innerHTML=h;document.getElementById('inline-summary-close').addEventListener('click',function(){inlineSummary.style.display='none';});}catch(err){inlineSummary.innerHTML='<p style="color:#c62828;text-align:center;padding:20px">'+escHtml(err.message)+'</p>';}summarizeBtn.disabled=false;summarizeBtn.textContent='Summarize with AI';});}
  if(exportBtn){exportBtn.addEventListener('click',function(){var t=transcriptText.value.trim();if(!t){showTranscriptMessage('No transcript to export.','#c62828');return;}var l=saveLabel.value.trim()||'Session Transcript';var c=saveTarget.options[saveTarget.selectedIndex].text;var md='# '+l+'\n\n**Course:** '+c+'\n**Date:** '+new Date().toLocaleDateString()+'\n\n---\n\n'+t+'\n';var b=new Blob([md],{type:'text/markdown'});var a=document.createElement('a');a.href=URL.createObjectURL(b);a.download=l.replace(/[^a-zA-Z0-9 ]/g,'').replace(/\s+/g,'-').toLowerCase()+'.md';a.click();URL.revokeObjectURL(a.href);showTranscriptMessage('Exported!','#2D8659');});}
  var HK='msib-session-history',hItems=document.getElementById('session-history-items'),hEmpty=document.getElementById('session-history-empty'),hClear=document.getElementById('session-history-clear');
  function loadH(){try{return JSON.parse(localStorage.getItem(HK))||[];}catch(e){return[];}}function saveH(d){localStorage.setItem(HK,JSON.stringify(d));}
  function addH(t,u){var h=loadH();var i=h.findIndex(function(x){return x.url===u;});if(i>=0)h.splice(i,1);h.unshift({type:t,url:u,title:t==='youtube'?'YT: '+u.slice(0,45):'Zoom',accessedAt:Date.now()});if(h.length>10)h.length=10;saveH(h);renderH();}
  function renderH(){if(!hItems)return;var h=loadH();hItems.innerHTML='';if(!h.length){if(hEmpty)hEmpty.style.display='';return;}if(hEmpty)hEmpty.style.display='none';h.forEach(function(e){var c=document.createElement('button');c.className='session-history-chip';c.innerHTML=(e.type==='youtube'?'&#9654;':'&#128247;')+' '+escHtml((e.title||e.url).slice(0,40));c.title=e.url;c.addEventListener('click',function(){urlInput.value=e.url;document.querySelectorAll('.sessions-type-btn').forEach(function(b){b.classList.remove('active');});var tb=document.querySelector('.sessions-type-btn[data-session-type="'+e.type+'"]');if(tb)tb.classList.add('active');connectBtn.click();});hItems.appendChild(c);});}
  if(hClear){hClear.addEventListener('click',function(){if(confirm('Clear history?')){saveH([]);renderH();}});}
  new MutationObserver(function(){if(playerArea.style.display!=='none'&&currentSession.url&&currentSession.type)addH(currentSession.type,currentSession.url);}).observe(playerArea,{attributes:true,attributeFilter:['style']});
  renderSavedList();
  renderH();
}
